...
FILEPATH = "/home/chady/shis/NG.txt";
...
def append_to_text_file(filename, text_to_append):
    try:
        ...
        with open(filename, 'w') as file:
            ...
            file.write('\n')
        print(f"String '{text_to_append}' appended to {filename}")
    except Exception as e:
        print(f"An error occurred: {e}")

def has_file_been_modified(filename, recorded_timestamp):
    try:
        # Get the current modification timestamp of the file
        current_timestamp = round(os.path.getmtime(filename))

        # Compare the current timestamp with the recorded timestamp
...
def run(model: str, num_hands: int,
        ...
        camera_id: int, width: int, height: int, recorded_timestamp: float, last_execution_time: float) -> None:
        ...
        if category_name != "None":
          ...
          #Adds newly recognized gesture to Text File
            if time.time() - last_execution_time >= 5:
              ...
              if has_file_been_modified(FILEPATH, recorded_timestamp):
                
                append_to_text_file(FILEPATH, category_name)
                recorded_timestamp = os.path.getmtime(FILEPATH)
                print("5 seconds have elapsed since the last execution.")
    
                # Update the last execution time
                last_execution_time = time.time()
                
                #Terminates program if ThumbsDown
                if category_name == "Thumb_Down":
                  sys.exit("Exiting due to Thumb Down")
...
def main():
  ...
  run(args.model, int(args.numHands), args.minHandDetectionConfidence,
      ...
      int(args.cameraId), args.frameWidth, args.frameHeight, recorded_timestamp, last_execution_time)
